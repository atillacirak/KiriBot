...
export function startDashboard(client, port = 3000) {
  ...
  function getUserDisplayRole(member, user, level, roleRewards = {}) {
    ...
    const isMichi = username.toLowerCase().includes('michi') || tag.toLowerCase().includes('michi') || userId === 'michi' || userId === '852613136595157072';
    if (hasRole('1315047438267973652', /mod/i) || isMichi) {
      const r = getRoleObj('1315047438267973652', /mod/i);
      const color = isMichi ? '#FF69B4' : ((r && r.hexColor && r.hexColor !== '#000000') ? r.hexColor : '#58F5FF');
      return {
        name: 'Moderatör',
        color: color,
        bg: `${color}25`,
    ...
    try {
      ...
      const richUsers = await Promise.all(
        users.map(async (u, index) => {
          let userObj = client.users ? client.users.cache.get(u.userId) : null;
          ...
          const avatar = memberObj ? memberObj.displayAvatarURL({ size: 128 }) : (userObj ? userObj.displayAvatarURL({ size: 128 }) : 'https://cdn.discordapp.com/embed/avatars/0.png');
